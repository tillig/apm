...
""")
    ...
    def test_write_path_preserves_unmanaged_manual_copilot_root_instructions(self, temp_project):
        ...
        assert any("APM marker" in w for w in result.warnings)
    ...
    def test_skip_warning_uses_relative_path_and_concrete_action(self, temp_project):
        ...
        skip_warnings = [
            w for w in result.warnings if "without an APM marker" in w or "APM marker" in w
        ]
        ...
        assert "Remove or rename" in warning, "warning must give a concrete action"
