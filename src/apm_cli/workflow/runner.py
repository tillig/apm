...
def run_workflow(workflow_name, params=None, base_dir=None):
    ...
    if workflow.llm_model and fallback_llm:
        print(
            f"{WARNING}WARNING: Both frontmatter 'llm: {workflow.llm_model}' and --llm '{fallback_llm}' specified. Using frontmatter value: {workflow.llm_model}{RESET}"
        )
    ...
    try:
        ...
        if runtime_name:
            ...
            if RuntimeFactory.runtime_exists(runtime_name):
                ...
                return (
                    False,
                    f"Invalid runtime '{runtime_name}'. Available runtimes: {', '.join(available_runtimes)}",
                )
        else:
        ...
        return False, f"Runtime execution failed: {e!s}"
